...
This walks the chain and reports the signal fraction each stage recovers:

1. the target itself, as the reference,
2. the unconstrained base flow, filtered to the band,
3. the SIREN's reconstruction of the band, as a mass IoU,
4. the amortized conditional model, conditioned on the band.
...
from constrained_fm.scripts.eval_bench1k import (build_parser as bench_parser, conditioning,
                                                 ...
                                                 resolve_defaults)
from constrained_fm.scripts.plot_bumphunt import BAND_INDEX, bench_namespace, signal_band
from constrained_fm.src.consts import BUMP_SIGNAL_SIGMA, BUMP_SIREN_TAU
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit signal recovery inside the band.")
...
def siren_iou(models: dict, constraint, problem, points: torch.Tensor, device) -> float:
    """Mass IoU between the SIREN's decoded region and the true band.
    ...
    """
    cond = conditioning("functa", models, constraint, BAND_INDEX, problem, device)
...
def main() -> None:
    ...
    print(f"band mass {mass * 100:.2f}%  |  tau {models['functa_cfg']['tau']} "
    ...
    if base is not None:
        ...
        print(f"base flow, filtered to band    {signal_fraction(target, inside):7.3f}%   "
              f"({inside.shape[0]} of 500000)")

    print(f"SIREN band reconstruction IoU  "
    ...
    for method in ("functa", "eci", "hardflow"):
        cond = conditioning(method, models, constraint, BAND_INDEX, problem, device)
        with torch.no_grad():
            samples = generate(method, models, constraint, BAND_INDEX, x0, problem,
        ...
        print(f"{method:<30} {recovered:7.3f}%   (AR {rate:.2f}%)")
