...
def generate_and_visualize_samples(model, num_samples=50000, step_size=0.05,
                                   bounds: list | None = None, coeffs: torch.Tensor = None, degree=POLYNOMIAL_DEGREE,
                                   scale=PLANE_SCALE, cluster_points=True):
    device = next(model.parameters()).device
    kwargs = {"bounds": bounds} if bounds is not None else {"coeffs": coeffs} if coeffs is not None else {}
    samples, T = model.sample(num_points=num_samples, **kwargs, step_size=step_size, return_intermediates=True, device=device)
...
def visualize_likelihood(likelihood, bounds=None, coeffs=None, degree=POLYNOMIAL_DEGREE, scale=PLANE_SCALE, grid_size=200, device=None):
    """
    Renders the exact log-likelihood array generated by the model and overlays the constraint boundaries.
    """
