...
def visualize_tiled_reconstruction(tiled_samples, active_boxes, gmm_true_pool, bounds_limit=4.5):
    """
    Side-by-side comparison of the Tiled Compound Generation vs the True GMM Target.
    Uses 2D histograms to visualize the empirical density.
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))

    samples_np = tiled_samples.cpu().numpy()
    true_np = gmm_true_pool.cpu().numpy()

    norm = mcolors.PowerNorm(gamma=0.6)  # Gamma correction makes lower densities more visible
    cmap = 'magma'
    bins = 150

    # --- LEFT PLOT: Tiled Compound Generation ---
    h1 = ax1.hist2d(samples_np[:, 0], samples_np[:, 1], bins=bins,
                    range=[[-bounds_limit, bounds_limit], [-bounds_limit, bounds_limit]],
                    cmap=cmap, norm=norm, density=True)

    # Overlay the active grid tiles
    ...
    ax1.set_title('Tiled Compound Generation\n(Empirical Density)', fontweight='bold')
    ax1.set_xlim(-bounds_limit, bounds_limit)
    ax1.set_ylim(-bounds_limit, bounds_limit)
    ax1.set_aspect('equal')

    # --- RIGHT PLOT: True Target Distribution ---
    h2 = ax2.hist2d(true_np[:, 0], true_np[:, 1], bins=bins,
                    range=[[-bounds_limit, bounds_limit], [-bounds_limit, bounds_limit]],
                    cmap=cmap, norm=norm, density=True)

    ax2.set_title('True GMM Target\n(Empirical Density)', fontweight='bold')
    ax2.set_xlim(-bounds_limit, bounds_limit)
    ax2.set_ylim(-bounds_limit, bounds_limit)
    ax2.set_aspect('equal')

    fig.colorbar(h1[3], ax=[ax1, ax2], orientation='vertical', label='Density', fraction=0.02)
